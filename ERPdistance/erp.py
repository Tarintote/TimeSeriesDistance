...
def distance(s1, s2, g=None, window=None, max_dist=None,
    ...
    for i in range(r):
        ...
        for j in range(j_start, j_end):
            ...
            cand_values = np.array(
                [erp[i0, j - skipp] + dij, erp[i0, j + 1 - skipp] + di, erp[i1, j - skip] + dj])
            ...
            # print('{}, {}, {}'.format(
            #    erp[i0, j - skipp], erp[i0, j + 1 - skipp], erp[i1, j - skip]))
            # print('i={}, j={}, dij={}, di={}, dj={}'.format(
            #    i, j, dij, di, dj))
            # print(erp)
...
def distance_fast(s1, s2, g=None, window=None, max_dist=None,
                  max_step=None, max_length_diff=None, penalty=None, psi=None):
...
def _distance_c_with_params(t):
    return erp_c.distance(t[0], t[1], **t[2])
...
def distance_matrix(s, g=None, max_dist=None, max_length_diff=None,
    ...
    if use_c and use_nogil:
        ...
        if parallel:
            ...
            dists = erp_c.distance_matrix_nogil_p(s, **dist_opts)
        else:
            logger.info("Use serial computation")
            dists = erp_c.distance_matrix_nogil(s, **dist_opts)
    if use_c and not use_nogil:
        ...
        if parallel:
            ...
            logger.info("Use serial computation")
            dist_opts['block'] = block
            dists = erp_c.distance_matrix(s, **dist_opts)
    if not use_c:
...
def distance_matrix_fast(s, g=None, max_dist=None, max_length_diff=None,
                         window=None, max_step=None, penalty=None, psi=None,
    ...
    if erp_c is None:
        _print_library_missing()
        return None
    return distance_matrix(s, g=g, max_dist=max_dist, max_length_diff=max_length_diff,
