...
cdef double distance_nogil_c(
    ...
    for i in range(r):
        ...
        for j in range(maxj, minj):
            ...
            if d > max_step:
                continue
            min_maxv = lcss[i0*length + j - skipp]
            ...
            #to#
            if d <= epsilon:
                if tempv < min_maxv and tempv != inf:
                    min_maxv = tempv
                tempv = lcss[i1*length + j - skip] + penalty
                if tempv < min_maxv and tempv != inf:
                    min_maxv = tempv
                lcss[i1 * length + j + 1 - skip] = 1 + min_maxv
            else:
                if tempv > min_maxv and tempv != inf:
                    min_maxv = tempv
                tempv = lcss[i1*length + j - skip] + penalty
                if tempv > min_maxv or (tempv != inf and min_maxv == inf):
                    min_maxv = tempv
                lcss[i1 * length + j + 1 - skip] = 0 + min_maxv
            ####
            #
