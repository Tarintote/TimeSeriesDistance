...
Dynamic Time Warping (ddtw), C implementation.
...
def distance(np.ndarray[DTYPE_t, ndim=1] s1, np.ndarray[DTYPE_t, ndim=1] s2,
    ...
    cdef np.ndarray[DTYPE_t, ndim=2] ddtw = np.full((2, length), inf)
    # ddtw[0, 0] = 0
    cdef int i
    ...
    for i in range(r):
        ...
        for j in range(j_start, j_end):
            ...
            if j == j_end - 1:
                n = j_end - 2
            dds1 = (s1[k] - s1[k-1]) + ((s1[k+1] - s1[k-1]*0.5))*0.5
            dds2 = (s2[n] - s2[n - 1]) + ((s2[n + 1] - s2[n - 1]*0.5))*0.5
            d = (dds1 - dds2)**2
            if d > max_step:
                continue
...
def distance_nogil(double[:] s1, double[:] s2,
    ...
    See distance(). This calls a pure c ddtw computation that avoids the GIL.
...
cdef double distance_nogil_c(
    ...
    See distance(). This is a pure c ddtw computation that avoid the GIL.
    """
    #printf("%i, %i\n", r, c)
    if max_length_diff != 0 and abs(r-c) > max_length_diff:
        return inf
    ...
    cdef int length = min(c+1,abs(r-c) + 2*(window-1) + 1 + 1 + 1)
    #printf("length (c) = %i\n", length)
    #cdef array.array ddtw_tpl = array.array('d', [])
    #cdef array.array ddtw
    #ddtw = array.clone(ddtw_tpl, length*2, zero=False)
    cdef double * ddtw
    ddtw = <double *> malloc(sizeof(double) * length * 2)
    cdef int i
    cdef int j
    ...
    for i in range(r):
        ...
        for j in range(maxj, minj):
            ...
            if d > max_step:
                continue
            minv = ddtw[i0*length + j - skipp]
            tempv = ddtw[i0*length + j + 1 - skipp] + penalty
            if tempv < minv:
                minv = tempv
            tempv = ddtw[i1*length + j - skip] + penalty
            if tempv < minv:
                minv = tempv
            #printf('d = %f, minv = %f\n', d, minv)
            ddtw[i1 * length + j + 1 - skip] = d + minv
            #
            #printf('%i, %i, %i\n',i0*length + j - skipp,i0*length + j + 1 - skipp,i1*length + j - skip)
            #printf('%f, %f, %f\n',ddtw[i0*length + j - skipp],ddtw[i0*length + j + 1 - skipp],ddtw[i1*length + j - skip])
            ...
            #printf("]\n")
            #
            if ddtw[i1*length + j + 1 - skip] <= max_dist:
                last_under_max_dist = j
            else:
                ddtw[i1*length + j + 1 - skip] = inf
                if prev_last_under_max_dist + 1 - skipp < j + 1 - skip:
                    break
        if last_under_max_dist == -1:
            # print('early stop')
        ...
        if psi != 0 and minj == c and r - 1 - i <= psi:
            if ddtw[i1*length + length - 1] < psi_shortest:
                psi_shortest = ddtw[i1*length + length - 1]
    ...
    if window - 1 < 0:
        c = c + window - 1
    cdef double result = sqrt(ddtw[length * i1 + c - skip])
    if psi != 0:
        for i in range(c - skip - psi, c - skip + 1):  # iterate over vci
            if ddtw[i1*length + i] < psi_shortest:
                psi_shortest = ddtw[i1*length + i]
...
def distance_matrix_nogil(cur, double max_dist=inf, int max_length_diff=0,
    ...
    """Compute a distance matrix between all sequences given in `cur`.
    This method calls a pure c implementation of the ddtw computation that
    avoids the GIL.
    """
