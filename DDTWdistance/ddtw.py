...
def distance_fast(s1, s2, window=None, max_dist=None,
    ...
    if psi is None:
        psi = 0
    d = ddtw_c.distance_nogil(s1, s2, window,
                              max_dist=max_dist,
                              max_step=max_step,
                              max_length_diff=max_length_diff,
                              penalty=penalty,
...
def _distance_c_with_params(t):
    return ddtw_c.distance(t[0], t[1], **t[2])
...
def warping_paths(s1, s2, window=None, max_dist=None,
    ...
    :returns: (ddtw distance, ddtw matrix)
    """
    r, c = len(s1), len(s2)
    if max_length_diff is not None and abs(r - c) > max_length_diff:
        return np.inf
...
def distance_matrix(s, max_dist=None, max_length_diff=None,
    ...
    if use_c and use_nogil:
        ...
        if parallel:
            ...
            dists = ddtw_c.distance_matrix_nogil_p(s, **dist_opts)
        else:
            logger.info("Use serial computation")
            dists = ddtw_c.distance_matrix_nogil(s, **dist_opts)
    if use_c and not use_nogil:
        ...
        if parallel:
            ...
            logger.info("Use serial computation")
            dist_opts['block'] = block
            dists = ddtw_c.distance_matrix(s, **dist_opts)
    if not use_c:
